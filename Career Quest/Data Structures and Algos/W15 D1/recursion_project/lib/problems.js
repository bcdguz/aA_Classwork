...
function sumArray(array) {
    ...
    return array.splice(array.length - 1, 1)[0] + sumArray(array);
}
...
function pow(base, exponent) {
    ...
    if (exponent > 0) {
        ...
        const posExp = exponent * -1;
        return 1/(base * pow(base, posExp - 1))
    }
}
...
function flatten(data) {
    ...
    data.forEach(item => {
        if (Array.isArray(item)) {
            ...
            flattened = flattened.concat(flatItem);
        } else {
            flattened.push(item);
        }
    })
...
function fileFinder(directories, targetFile) {
    ...
    for (let key in directories) {
        ...
        const searchResult = fileFinder(directories[key], targetFile);
...
function pathFinder(directories, targetFile) {
    ...
    for (let key in directories) {
        ...
        const searchResult = pathFinder(directories[key], targetFile);
        if (searchResult != null) return key + searchResult;
    }
    return null;
}
