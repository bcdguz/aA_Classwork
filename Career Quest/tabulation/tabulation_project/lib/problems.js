...
function maxNonAdjacentSum(nums) {
    ...
    for (let i = 1; i < table.length; i++) {
        let skipped = table[i - 2] === undefined ? 0 : table[i - 2]; //maxNonAdjacentSum up to here
        let includeCurr = skipped + nums[i];
        let excludeCurr = table[i-1]; //maxNonAdjascentSum up to one before
        table[i] = Math.max(includeCurr, excludeCurr);
    }
...
function minChange(coins, amount) {
    let table = new Array(amount + 1).fill(Infinity); //min coins to make that amount
    table[0] = 0;

    coins.forEach(coin => {
        for (let amnt = 0; amnt < table.length; amnt++) { //Iterating through our table
            for (let quant = 0; quant * coin <= amnt; quant++) { //Possible quantities of coin
                const remainder = amnt - (quant * coin);
                const minCoins = table[remainder] + quant; //min amnt of coins to make remainder + current quant
                if (minCoins < table[amnt]) table[amnt] = minCoins;
            }
        }
    })
